  ...
  protected override async Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        if (string.IsNullOrEmpty(authorizationHeader) || !authorizationHeader.StartsWith("Bearer ")) return AuthenticateResult.Fail("No token");
        ...
        try
        {
            ...
            if (null == dbUser) return AuthenticateResult.Fail("Invalid token");
        }
        catch (Exception ex)
        {
